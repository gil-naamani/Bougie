...
app.directive('categorySpendTotals', function(){

	TEMPLATE = `
	<div class="widget-container col-md-10 col-md-offset-1">
		<nvd3 options="categoryTotalsController.options" data="categoryTotalsController.data" api="api"></nvd3>
	</div>`

	return {
		restrict 		 : 'E',
		template 		 : TEMPLATE,
		controller 		 : 'categorySpendTotalsController',
		controllerAs 	 : 'categoryTotalsController',
		...
		link : function(scope, element, attrs) {
		    ...
		    // 	scope.categoryTotalsController.setData(newVal)
		    // }, true);
		}
	}
});
...
app.controller('categorySpendTotalsController', ['$scope', 'modalService', function($scope, modalService){
    ...
    this.setData = function(){
        ...
        for (var category in dataMap) {
            ...
            for (var d = new Date(y, m, 1); d <= new Date(y, m + 1, 0); d.setDate(d.getDate() + 1)) {
                ...
                for (var i = 0; i < dataMap[category].length; i++) {
                    var expense = dataMap[category][i];
                    var date = new Date(Date.parse(expense.timestamp));
                    var eMonth = date.getMonth()
                    var eDay = date.getDate();

                    if (eMonth == d.getMonth() && eDay == d.getDate()){
                        sum += expense.amt;
                    }
                }
                series.values.push([sum, time * 1000])
            }
